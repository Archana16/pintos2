...
static void syscall_handler(struct intr_frame *f) {
	...
	static const struct syscall syscall_table[] = { { 0,
			(syscall_function *) sys_halt },
			{ 1, (syscall_function *) sys_exit }, { 1,
					(syscall_function *) sys_exec }, { 1,
					(syscall_function *) sys_wait }, { 2,
					(syscall_function *) sys_create }, { 1,
					(syscall_function *) sys_remove }, { 1,
					(syscall_function *) sys_open }, { 1,
					(syscall_function *) sys_filesize }, { 3,
					(syscall_function *) sys_read }, { 3,
					(syscall_function *) sys_write }, { 2,
...
int mmap(int handle, void *addr) {
	...
	int32_t ofs = 0;
	uint32_t read_bytes = sys_filesize(handle);
	while (read_bytes > 0) {
