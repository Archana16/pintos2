...
static void start_process(void *exec_) {
	...
	if (success) {
		exec->wait_status = thread_current()->wait_status = malloc(
				sizeof *exec->wait_status);
...
int process_wait(tid_t child_tid) {
	...
	for (e = list_begin(&cur->children); e != list_end(&cur->children); e =
			list_next(e)) {
...
static bool init_cmd_line(uint8_t *kpage, uint8_t *upage, const char *cmd_line,
		void **esp) {
	size_t ofs = PGSIZE;
	...
	argc = 0;
	for (karg = strtok_r(cmd_line_copy, " ", &saveptr); karg != NULL; karg =
